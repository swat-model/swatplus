     ...
     if (pco%salt_basin%d == "y" .and. cs_db%num_salts > 0) then
       ...
       write(5080,*) 
       write(5080,*) 'lat_kg:       kg      salt mass in soil lateral flow            (soil --> channel)'
       write(5080,*) 'gw_kg:        kg      salt mass in groundwater flow             (aquifer --> channel)'
       write(5080,*) 'sur_kg:       kg      salt mass in surface runoff               (soil --> channel)'
       write(5080,*) 'urb_kg:       kg      salt mass in urban runoff                 (soil --> channel)'
       write(5080,*) 'wet_kg:       kg      salt mass in wetland runoff               (wetland --> channel)'
       write(5080,*) 'tile_kg:      kg      salt mass in tile drain flow              (soil --> channel)'
     ...
     if (pco%salt_basin%m == "y" .and. cs_db%num_salts > 0) then
       ...
       write(5082,*) 
       write(5082,*) 'lat_kg:       kg      salt mass in soil lateral flow            (soil --> channel)'
       write(5082,*) 'gw_kg:        kg      salt mass in groundwater flow             (aquifer --> channel)'
       write(5082,*) 'sur_kg:       kg      salt mass in surface runoff               (soil --> channel)'
       write(5082,*) 'urb_kg:       kg      salt mass in urban runoff                 (soil --> channel)'
       write(5082,*) 'wet_kg:       kg      salt mass in wetland runoff               (wetland --> channel)'
       write(5082,*) 'tile_kg:      kg      salt mass in tile drain flow              (soil --> channel)'
     ...
     if (pco%salt_basin%y == "y" .and. cs_db%num_salts > 0) then
       ...
       write(5084,*) 
       write(5084,*) 'lat_kg:       kg      salt mass in soil lateral flow            (soil --> channel)'
       write(5084,*) 'gw_kg:        kg      salt mass in groundwater flow             (aquifer --> channel)'
       write(5084,*) 'sur_kg:       kg      salt mass in surface runoff               (soil --> channel)'
       write(5084,*) 'urb_kg:       kg      salt mass in urban runoff                 (soil --> channel)'
       write(5084,*) 'wet_kg:       kg      salt mass in wetland runoff               (wetland --> channel)'
       write(5084,*) 'tile_kg:      kg      salt mass in tile drain flow              (soil --> channel)'
     ...
     if (pco%salt_basin%a == "y" .and. cs_db%num_salts > 0) then
       ...
       write(5086,*) 
       write(5086,*) 'lat_kg:       kg/yr   salt mass in soil lateral flow            (soil --> channel)'
       write(5086,*) 'gw_kg:        kg/yr   salt mass in groundwater flow             (aquifer --> channel)'
       write(5086,*) 'sur_kg:       kg/yr   salt mass in surface runoff               (soil --> channel)'
       write(5086,*) 'urb_kg:       kg/yr   salt mass in urban runoff                 (soil --> channel)'
       write(5086,*) 'wet_kg:       kg/yr   salt mass in wetland runoff               (wetland --> channel)'
       write(5086,*) 'tile_kg:      kg/yr   salt mass in tile drain flow              (soil --> channel)'
     ...
     if (pco%salt_hru%d == "y" .and. cs_db%num_salts > 0) then
       ...
       write(5021,*) 'surq:         kg/ha   salt mass in surface runoff            (soil --> channel)'
       write(5021,*) 'latq:         kg/ha   salt mass in soil lateral flow         (soil --> channel)'
       write(5021,*) 'urbq:         kg/ha   salt mass in urban runoff              (soil --> channel)'
       write(5021,*) 'wetq:         kg/ha   salt mass in wetland runoff            (wetland --> channel)'
       write(5021,*) 'tile:         kg/ha   salt mass in tile drainage             (soil --> channel)'
     ...
     if (pco%salt_hru%m == "y" .and. cs_db%num_salts > 0) then
       ...
       write(5023,*) 'surq:         kg/ha   salt mass in surface runoff            (soil --> channel)'
       write(5023,*) 'latq:         kg/ha   salt mass in soil lateral flow         (soil --> channel)'
       write(5023,*) 'urbq:         kg/ha   salt mass in urban runoff              (soil --> channel)'
       write(5023,*) 'wetq:         kg/ha   salt mass in wetland runoff            (wetland --> channel)'
       write(5023,*) 'tile:         kg/ha   salt mass in tile drainage             (soil --> channel)'
     ...
     if (pco%salt_hru%y == "y" .and. cs_db%num_salts > 0) then
       ...
       write(5025,*) 'surq:         kg/ha   salt mass in surface runoff            (soil --> channel)'
       write(5025,*) 'latq:         kg/ha   salt mass in soil lateral flow         (soil --> channel)'
       write(5025,*) 'urbq:         kg/ha   salt mass in urban runoff              (soil --> channel)'
       write(5025,*) 'wetq:         kg/ha   salt mass in wetland runoff            (wetland --> channel)'
       write(5025,*) 'tile:         kg/ha   salt mass in tile drainage             (soil --> channel)'
     ...
     if (pco%salt_hru%a == "y" .and. cs_db%num_salts > 0) then
       ...
       write(5027,*) 'surq:         kg/ha/yr   salt mass in surface runoff            (soil --> channel)'
       write(5027,*) 'latq:         kg/ha/yr   salt mass in soil lateral flow         (soil --> channel)'
       write(5027,*) 'urbq:         kg/ha/yr   salt mass in urban runoff              (soil --> channel)'
       write(5027,*) 'wetq:         kg/ha/yr   salt mass in wetland runoff            (wetland --> channel)'
       write(5027,*) 'tile:         kg/ha/yr   salt mass in tile drainage             (soil --> channel)'
     ...
     if (sp_ob%aqu > 0) then
       if (pco%salt_aqu%d == "y" .and. cs_db%num_salts > 0) then
         ...
         write(5060,*) 
         write(5060,*) 'gw:           kg      salt mass in groundwater discharge        (aquifer --> channel)' 
     ...
     if (sp_ob%aqu > 0) then
       if (pco%salt_aqu%m == "y" .and. cs_db%num_salts > 0) then
         ...
         write(5062,*) 
         write(5062,*) 'gw:           kg      salt mass in groundwater discharge        (aquifer --> channel)' 
     ...
     if (sp_ob%aqu > 0) then
       if (pco%salt_aqu%y == "y" .and. cs_db%num_salts > 0) then
         ...
         write(5064,*) 
         write(5064,*) 'gw:           kg      salt mass in groundwater discharge        (aquifer --> channel)' 
     ...
     if (sp_ob%aqu > 0) then
       if (pco%salt_aqu%a == "y" .and. cs_db%num_salts > 0) then
         ...
         write(5066,*) 
         write(5066,*) 'gw:           kg/yr   salt mass in groundwater discharge        (aquifer --> channel)' 
     ...
     if (sp_ob%ru > 0) then
       if (pco%salt_ru%d == "y" .and. cs_db%num_salts > 0) then
         ...
         write(5070,*) 'surq:         kg      salt mass in surface runoff            (soil --> channel)'
         write(5070,*) 'latq:         kg      salt mass in soil lateral flow         (soil --> channel)'
         write(5070,*) 'tile:         kg      salt mass in tile drainage             (soil --> channel)'
     ...
     if (sp_ob%ru > 0) then
       if (pco%salt_ru%m == "y" .and. cs_db%num_salts > 0) then
         ...
         write(5072,*) 'surq:         kg      salt mass in surface runoff            (soil --> channel)'
         write(5072,*) 'latq:         kg      salt mass in soil lateral flow         (soil --> channel)'
         write(5072,*) 'tile:         kg      salt mass in tile drainage             (soil --> channel)'
     ...
     if (sp_ob%ru > 0) then
       if (pco%salt_ru%y == "y" .and. cs_db%num_salts > 0) then
         ...
         write(5074,*) 'surq:         kg      salt mass in surface runoff            (soil --> channel)'
         write(5074,*) 'latq:         kg      salt mass in soil lateral flow         (soil --> channel)'
         write(5074,*) 'tile:         kg      salt mass in tile drainage             (soil --> channel)'
     ...
     if (sp_ob%ru > 0) then
       if (pco%salt_ru%a == "y" .and. cs_db%num_salts > 0) then
         ...
         write(5076,*) 'surq:         kg/yr   salt mass in surface runoff            (soil --> channel)'
         write(5076,*) 'latq:         kg/yr   salt mass in soil lateral flow         (soil --> channel)'
         write(5076,*) 'tile:         kg/yr   salt mass in tile drainage             (soil --> channel)'
