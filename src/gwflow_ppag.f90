      subroutine gwflow_ppag(ob_id_src,ob_id_dmd,demand_vol,extracted,dmd_unmet,demand_type) !rtb gwflow
...
!!    (pumping volume are used in gwflow_simulate, in groundwater balance equations)
      ...
      integer, intent (in) :: ob_id_src	    !       |object designated as the water source
	  integer, intent (in) :: ob_id_dmd	    !       |object that has a water demand
	  ...
	  character*4, intent (in) :: demand_type
      integer :: i = 0                      !       |counter
      ...
      hru_id = ob_id_dmd
        ...
        do i=1,hru_num_cells(hru_id)
          ...
          gw_ss_sum(cell_id)%ppag = gw_ss_sum(cell_id)%ppag + (gwvol_removed * (-1)) !store for annual water
          gw_ss_sum_mo(cell_id)%ppag = gw_ss_sum_mo(cell_id)%ppag + (gwvol_removed * (-1)) !store for monthly water
          ...
          if(gw_heat_flag) then
            ...
            gw_heat_ss_sum(cell_id)%ppag = gw_heat_ss_sum(cell_id)%ppag + heat_flux * (-1)
          endif
      ...
      if(hru_num_cells(hru_id) > 0) then
        ...
        do i=1,hru_num_cells(hru_id)
          ...
          !save the pumping volume (m3), for use in gwflow_simulate
          gw_ss(cell_id)%ppex = gwvol_removed * (-1) !m3 negative = leaving the aquifer
          gw_ss_sum(cell_id)%ppex = gw_ss_sum(cell_id)%ppex + (gwvol_removed * (-1)) !store for annual water
          gw_ss_sum_mo(cell_id)%ppex = gw_ss_sum_mo(cell_id)%ppex + (gwvol_removed * (-1)) !store for monthly water
          ...
          if(gw_heat_flag) then
            ...
            gw_heat_ss_sum(cell_id)%ppex = gw_heat_ss_sum(cell_id)%ppex + heat_flux * (-1)
          endif
