        ...
        if(hru_num_cells(hru_id) > 0) then
          ...
          do icell=1,hru_num_cells(hru_id)
            ...
            if(wt > wet_stage) then !groundwater inflow to wetland
              ...
              gw_ss_sum(cell_id)%wetl = gw_ss_sum(cell_id)%wetl + (gw_inflow*(-1)) !store for annual water
              gw_ss_sum_mo(cell_id)%wetl = gw_ss_sum_mo(cell_id)%wetl + (gw_inflow*(-1)) !store for monthly water
              ...
              if(gw_heat_flag) then
                ...
                gw_heat_ss_sum(cell_id)%wetl = gw_heat_ss_sum(cell_id)%wetl + (heat_flux*(-1))
              endif
